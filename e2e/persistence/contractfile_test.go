...
func TestContractFileMatchesRevision(t *testing.T) {
	...
	record(t, caseInput{ID: "process/contract-file-sha256", Contract: "QA gate", Kind: "process",
		Description: "the shipped contract file is byte-identical to " + contractRevision,
		...
		Expected:    map[string]any{"sha256": contractSHA256, "lines": contractLines, "read": "ok"},
		Actual:      map[string]any{"sha256": full, "lines": lines, "read": sqlState(readErr)},
		Pass:        readErr == nil && full == contractSHA256 && lines == contractLines})
}
